...
// Theme-aware chart ink via CSS variables (resolve in inline styles, not SVG attrs).
const GRID = 'var(--line-2)'
const AXIS_TEXT = 'var(--fg-muted)'
const DOT_RING = 'var(--ink-1)'
...
export function BacChart({
  ...
  return (
    ...
    >
      ...
      {gridLines.map((v) => (
        <g key={v}>
          <line x1={leftPad} y1={y(v)} x2={width - rightPad} y2={y(v)} style={{ stroke: GRID }} strokeOpacity={0.5} />
          <text x={leftPad - 6} y={y(v) + 3} textAnchor="end" fontSize={9} style={{ fill: AXIS_TEXT }}>
            {v.toFixed(1)}
          </text>
        </g>
      ))}
      ...
      <polyline points={targetPath} fill="none" style={{ stroke: ZONE.green }} strokeWidth={2.5} strokeDasharray="5 4" />
      ...
      {pts.map((p, i) => {
        ...
        return (
          <g key={p.id ?? i}>
            <line x1={x(i)} y1={y(p.optimalBAC)} x2={x(i)} y2={y(p.bac)} style={{ stroke: color }} strokeOpacity={0.8} strokeWidth={2} />
            <circle cx={x(i)} cy={y(p.bac)} r={dotR} style={{ fill: color, stroke: DOT_RING }} strokeWidth={compact ? 1 : 1.5} />
            {!compact && (
              <text x={x(i)} y={height - bottomPad + 16} textAnchor="middle" fontSize={9} style={{ fill: AXIS_TEXT }}>
                R{p.roundNumber}
              </text>
            )}
          </g>
